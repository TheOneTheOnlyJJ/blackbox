...
import { IUserAuthAPI, UserAuthAPIIPCChannel, USER_AUTH_API_IPC_CHANNELS, SignedInUserChangedCallback } from "@shared/IPC/APIs/UserAuthAPI";
...
const PRELOAD_IPC_USER_AUTH_API_LOG_SCOPE = "p-ipc-uauth-api";

export const USER_AUTH_API_PRELOAD_HANDLERS: IUserAuthAPI = {
  signUp: (encryptedUserSignUpDTO: IEncryptedData<IUserSignUpDTO>): IPCAPIResponse<boolean> => {
    const CHANNEL: UserAuthAPIIPCChannel = USER_AUTH_API_IPC_CHANNELS.signUp;
  ...
  signIn: (encryptedUserSignInDTO: IEncryptedData<IUserSignInDTO>): IPCAPIResponse<boolean> => {
    const CHANNEL: UserAuthAPIIPCChannel = USER_AUTH_API_IPC_CHANNELS.signIn;
    sendLogToMainProcess(PRELOAD_IPC_USER_AUTH_API_LOG_SCOPE, "debug", `Messaging main on channel: "${CHANNEL}".`);
  ...
  signOut: (): IPCAPIResponse<ISignedInUserInfo | null> => {
    const CHANNEL: UserAuthAPIIPCChannel = USER_AUTH_API_IPC_CHANNELS.signOut;
    sendLogToMainProcess(PRELOAD_IPC_USER_AUTH_API_LOG_SCOPE, "debug", `Messaging main on channel: "${CHANNEL}".`);
  ...
  isUsernameAvailable: (username: string): IPCAPIResponse<boolean> => {
    const CHANNEL: UserAuthAPIIPCChannel = USER_AUTH_API_IPC_CHANNELS.isUsernameAvailable;
    sendLogToMainProcess(PRELOAD_IPC_USER_AUTH_API_LOG_SCOPE, "debug", `Messaging main on channel: "${CHANNEL}".`);
  ...
  getSignedInUserInfo: (): IPCAPIResponse<ISignedInUserInfo | null> => {
    const CHANNEL: UserAuthAPIIPCChannel = USER_AUTH_API_IPC_CHANNELS.getSignedInUserInfo;
    sendLogToMainProcess(PRELOAD_IPC_USER_AUTH_API_LOG_SCOPE, "debug", `Messaging main on channel: "${CHANNEL}".`);
    return ipcRenderer.sendSync(CHANNEL) as IPCAPIResponse<ISignedInUserInfo | null>;
  },
