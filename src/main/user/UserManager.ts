...
export class UserManager {
  ...
  public signInUser(userSignInCredentials: IUserSignInCredentials): boolean {
    ...
    const USER_ID: UUID | null = this.userAccountStorage.getUserId(userSignInCredentials.username);
    if (USER_ID === null) {
      this.logger.debug("No user ID for given username. Username must be missing from storage. Sign in failed.");
      return false;
    }
    const USER_PASSWORD_DATA: [Buffer, Buffer] | null = this.userAccountStorage.getPasswordData(USER_ID);
  ...
  public addUserDataStorageConfig(userId: UUID, userDataStorageConfig: UserDataStorageConfig): boolean {
    ...
    return this.userAccountStorage.addUserDataStorageConfig(userId, userDataStorageConfig);
  }
  ...
  public getAllUserDataStorageConfigs(userId: UUID): UserDataStorageConfig[] {
    ...
    return this.userAccountStorage.getAllUserDataStorageConfigs(userId);
  }
}
