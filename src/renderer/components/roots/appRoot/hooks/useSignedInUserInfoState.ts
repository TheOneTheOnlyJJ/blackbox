...
export const useSignedInUserInfoState = (
  ...
  useEffect((): (() => void) => {
    const GET_SIGNED_IN_USER_INFO_RESPONSE: IPCAPIResponse<ISignedInUserInfo | null> = window.userAuthAPI.getSignedInUserInfo();
    ...
    const removeOnSignedInUserChangedListener: () => void = window.userAuthAPI.onSignedInUserChanged(
